 ...
 */
package br.uff.ic.dm.verde20221.actors;

import br.uff.ic.dm.verde20221.PlayerData;
import br.uff.ic.dm.verde20221.game.World;
import br.uff.ic.dm.verde20221.game.WorldController;
import br.uff.ic.dm.verde20221.game.ClassThreadComandos;
import br.uff.ic.dm.verde20221.pathfinder.FlatTiledNode;
...
public class Avatar extends Sprite {
    ...
    // path planning
    private ArrayList<FlatTiledNode> nodes;
    private int ind;
    ...
    public void update(float delta) {
        ...
        //Touch movement
        if(WorldController.clicado && this.getAuthUID() == PlayerData.myPlayerData().getAuthUID()) {
            //Gdx.app.log("Player ", " clicado x="+this.getX() + " y="+this.getY());
            this.nodes.clear(); //limpa os nos atuais
            if(World.world.pathPlan.path.getCount()!=0){ // se estiver com algum no
                int i = 0;
                for(FlatTiledNode node : World.world.pathPlan.path.nodes){
                    this.nodes.add(node); // adiciona aos nos do Avatar
                    i += 1;
                }
            }
            if(houveColisao == false && World.world.pathPlan.path.getCount()>1){ // se tiver mais de 1 no, ande
                if(this.nodes.size()>1){

                    if(this.ind<this.nodes.size()){
                        WorldController.target.x = this.nodes.get(this.ind).x * 32 + (32 / 2f);
                        WorldController.target.y = this.nodes.get(this.ind).y * 32 + (32 / 2f);
                    }
                    ClassThreadComandos.objetoAndroidFireBase.sendMoveTo(WorldController.target.x, WorldController.target.y); // envia um MOVETO pra cada no
                    //current.lerp(new Vector3(WorldController.target.x, WorldController.target.y, 0), 0.005f/world.getCamera().zoom);
                    double distancia = Math.sqrt((this.current.x - WorldController.target.x) * (this.current.x - WorldController.target.x) + (this.current.y - WorldController.target.y) * (this.current.y - WorldController.target.y));
                    this.vetorUnitarioMovimento.x = (float) ((WorldController.target.x - this.current.x) / distancia);
                    this.vetorUnitarioMovimento.y = (float) ((WorldController.target.y - this.current.y) / distancia);
                    double angulo1 = Math.acos((double) this.vetorUnitarioMovimento.x) * 180.0 / Math.PI; // [-90.0 .. 90.0]
                    double angulo2 = Math.asin((double) this.vetorUnitarioMovimento.y) * 180.0 / Math.PI; // [0.0 .. 180.0]
                    if (angulo2 >= 0.0)
                        this.playerMovementAngle = angulo1;
                    else
                        this.playerMovementAngle = 360.0 - angulo1;
                    defineOrientation(playerMovementAngle);


                    // TODO: descubra o erro: a velocidade está o dobro da prevista
                    double posx = (WorldController.target.x - this.current.x) / distancia * Avatar.SPEED * delta + this.current.x;
                    double posy = (WorldController.target.y - this.current.y) / distancia * Avatar.SPEED * delta + this.current.y;
                    this.current.x = (float) posx;
                    this.current.y = (float) posy;
                    this.setPosition(this.current.x, this.current.y);
                    this.setState(State.WALKING);
                }
                if (this.current.dst(WorldController.target.x, WorldController.target.y, 0) < 32 / 2 && WorldController.target.x == this.nodes.get(this.nodes.size()-1).x * 32 + (32 / 2f) && WorldController.target.y == this.nodes.get(this.nodes.size()-1).y * 32 + (32 / 2f)) {
                    WorldController.clicado = false; // se chegou, pare e fique IDLE
                    this.ind=0;
                    this.setState(State.IDLE);
                }
                if(this.current.x - WorldController.target.x <=20 && this.current.x - WorldController.target.x >=-20){
                    this.current.x = WorldController.target.x;
                }
                if(this.current.y - WorldController.target.y <=20 && this.current.y - WorldController.target.y >=-20){
                    this.current.y = WorldController.target.y;
                }
                if(this.current.x == WorldController.target.x && this.current.y == WorldController.target.y){
                    ind+=1;
                }
            } else {
                WorldController.clicado = false; // se tiver menos de 1 no, nao ande e fique IDLE
                this.ind=0;
                this.setState(State.IDLE);
            }
        }

        // o movimento do amigo nao eh processado pelo pathPlanning
        // porque ja vem calculado pelo firebase, cada no torna-se um MOVETO enviado acima
        if(WorldController.friendsClicado && this.getAuthUID() != PlayerData.myPlayerData().getAuthUID()) { // caso clicado do amigo seja True e o avatar que esta lendo isso eh o do Amigo
            //Gdx.app.log("Player ", " clicado x="+this.getX() + " y="+this.getY());
            if(houveColisao == false){
                //current.lerp(new Vector3(WorldController.target.x, WorldController.target.y, 0), 0.005f/world.getCamera().zoom);
                double distancia = Math.sqrt((current.x - WorldController.friendsTarget.x)*(current.x - WorldController.friendsTarget.x) + (current.y - WorldController.friendsTarget.y)*(current.y - WorldController.friendsTarget.y));
                vetorUnitarioMovimento.x = (float) ((WorldController.friendsTarget.x -current.x)/distancia);
                vetorUnitarioMovimento.y = (float) ((WorldController.friendsTarget.y -current.y)/distancia);
                ...
                // TODO: descubra o erro: a velocidade está o dobro da prevista
                double posx = (WorldController.friendsTarget.x -current.x)/distancia * Avatar.SPEED*delta  + current.x;
                double posy = (WorldController.friendsTarget.y -current.y)/distancia * Avatar.SPEED*delta  + current.y;
                current.x = (float) posx;
                current.y = (float) posy;
                this.setPosition(current.x, current.y);
                this.setState(State.WALKING);
            }
            if(this.current.dst(WorldController.friendsTarget.x, WorldController.friendsTarget.y, 0) < 32/2) {
                WorldController.friendsClicado = false;
                ClassThreadComandos.isMoving = false;
                this.setState(State.IDLE);
            }
        }
