...
//import br.uff.ic.dm.lek.actors.PlayerLife;
import br.uff.ic.dm.verde20221.utils.CameraZoomAdjust;
import br.uff.ic.dm.verde20221.utils.ClassToast;
import br.uff.ic.dm.verde20221.utils.PathPlanning;
...
//    }
    ...
    public static void load(String mapName, Alquimia alquimia) { // Change map method
        world = new World(mapName); // private constructor
        ...
        world.batch = new SpriteBatch();
        World.hud = new ClassHud(world.batch, alquimia);
        world.controller = new CameraController();
        world.gestureDetector = new GestureDetector(20, 0.5f, 2, 0.15f, world.controller);
        world.inputMultiplexer = new InputMultiplexer(); // Order matters here!
        world.inputMultiplexer.addProcessor(World.hud.stage); // 1st priority
        ...
        world.font.setColor(Color.RED);
        // LEK metodo deprecado world.font.scale(2);
        mensagem=PlayerData.myPlayerData().getPlayerNickName();
        //Gdx.app.log("Multitouch", "multitouch supported: " + Gdx.input.isPeripheralAvailable(Peripheral.MultitouchScreen));
        world.cameraZoomAdjust = new CameraZoomAdjust(0.1f, Gdx.graphics.getWidth(), Gdx.graphics.getHeight());
    ...
    public void render(float delta) {
        ...
        World.world.getAvatar().setNickName(PlayerData.myPlayerData().getPlayerNickName());
        String userNickName = World.world.getAvatar().getNickName();
        mensagem = userNickName;
        ...
        //this.world.getAvatar().rotate90(true);
        mensagem = "";
        if (!friendsAvatar.getNickName().equals("")) {
            mensagem = friendsAvatar.getNickName();
        }
        World.world.getFriendsAvatar().draw(camera, font, mensagem, this.tiledMapRender.getBatch());
    ...
    private Avatar friendsAvatar;

    public void setFriendsUID(String friendsUID) {
        this.friendsAvatar.setAuthUID(friendsUID);
    }

    public void setFriendsNickName(String friendsNickName) {
        this.friendsAvatar.setNickName(friendsNickName);
    }

    public Avatar getFriendsAvatar() {
    ...
    public PathPlanning friendsPathPlan;
    ...
    private World() {
        ...
        this.map = new TmxMapLoader().load("maps/Tilesets/MysteryDungeon.tmx");
        ...
        this.assets.load("img/guerreiraSW_NW_SE_NE_210x280.pack", TextureAtlas.class);
        this.assets.finishLoading();
        TextureAtlas atlasPlayerS_W_E_N = this.assets.get("img/guerreiraS_W_E_N_210x280.pack");
        TextureAtlas atlasPlayerSW_NW_SE_NE = this.assets.get("img/guerreiraSW_NW_SE_NE_210x280.pack");

        this.friendsAvatar = new Avatar(new Sprite(atlasPlayerS_W_E_N.findRegion("South02")), 2500, 700, null);
        ClassThreadComandos.objetoAndroidFireBase.getPlayerUID();
        ...
        ClassThreadComandos.objetoAndroidFireBase.sendInitialXY(avatarStartTileX*World.tileWidth, avatarStartTileY*World.tileHeight);

    }

    private World(String mapName) {
        ClassToast.initToastFactory();
        Color backgroundColor = new Color(0f, 0f, 0f, 0.5f);
        Color fontColor = new Color(1, 1, 0, 0.5f);
        String msg = "Welcome to " + mapName;
        ClassToast.toastRich(msg, backgroundColor, fontColor, 2f);
        //backgroundColor = new Color(1f, 1f, 1f, 0.5f);
        //fontColor = new Color(1, 0, 1, 0.5f);
        //msg = "outro texto";
        ...
        this.map = new TmxMapLoader().load("maps/Tilesets/" + mapName + ".tmx");
        //this.map = new TmxMapLoader().load("maps/all.tmx");
        //this.map.getLayers().get("Collision").setVisible(true);// mostra ou não os tiles de colisao
        World.bounds = new ArrayList<Rectangle>();
        xTiles = map.getProperties().get("width", Integer.class);
