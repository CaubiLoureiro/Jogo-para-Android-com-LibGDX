...
import br.uff.ic.dm.verde20221.actors.Avatar.State;
...
public class WorldController implements InputProcessor {
    ...
    public static Vector3 friendsVec;
    public static Vector3 target;
    public static Vector3 friendsTarget;
    public static boolean clicado;
    public static boolean friendsClicado;
    ...
    public WorldController(World world) {
        ...
        this.friendsAvatar = this.world.getFriendsAvatar();
    }
    ...
    public boolean touchUp(int screenX, int screenY, int pointer, int button) {
        ...
        if(requestMove){
            ...
            System.out.println("- AMIGO: " + World.world.getFriendsAvatar().getAuthUID());
    ...
    public void comandoMoveTo(float moveToX, float moveToY){
        float x = this.friendsAvatar.getX();
        float y = this.friendsAvatar.getY();
        //if(requestMove){
        this.world.getFriendsAvatar().setState(State.WALKING);
        WorldController.friendsVec = new Vector3(moveToX, moveToY, 0);
        WorldController.friendsTarget = new Vector3(friendsVec);

        // invoca o jogador para dar a direcao do personagem
        if (Math.abs(WorldController.friendsTarget.x - x) > Math.abs(WorldController.friendsTarget.y - y)){
            // movimento preponderante em x
            if ((WorldController.friendsTarget.x - x) > 0){
                // movimento para direita
                this.friendsAvatar.setOrientation(Avatar.Compass.EAST);
            } else {
                this.friendsAvatar.setOrientation(Avatar.Compass.WEST);
            }
        } else {
            // movimento preponderante em y
            if ((WorldController.friendsTarget.y - y) > 0){
                // movimento para cima
                this.friendsAvatar.setOrientation(Avatar.Compass.NORTH);
            } else {
                this.friendsAvatar.setOrientation(Avatar.Compass.SOUTH);
            }
        }

        WorldController.friendsClicado = true;
        fezTouchDown = false;
    }
