 ...
 */
package br.uff.ic.dm.verde20221.utils;

import br.uff.ic.dm.verde20221.game.World;
//https://github.com/libgdx/gdx-ai/wiki
import br.uff.ic.dm.verde20221.pathfinder.FlatTiledGraph;// necessário no projeto
import br.uff.ic.dm.verde20221.pathfinder.FlatTiledNode;// necessário no projeto
import br.uff.ic.dm.verde20221.pathfinder.TiledManhattanDistance;// necessário no projeto
import br.uff.ic.dm.verde20221.pathfinder.TiledRaycastCollisionDetector;// necessário no projeto
import br.uff.ic.dm.verde20221.pathfinder.TiledSmoothableGraphPath; // necessário no projeto
...
public class PathPlanning {
    ...
    int startTileX;
    int startTileY;
    public static TiledSmoothableGraphPath<FlatTiledNode> path;
    ...
    private int[][] pathMap;
    //variavel
    public ArrayList<FlatTiledNode> nodes;
    ...
    public void create() {
        ...
        lastScreenX = this.lastEndTileX * (int) tileWidth;
        lastScreenY = this.lastEndTileY * (int) tileWidth;
        // Create the map
        ...
        for (int i = 0; i < World.xTiles; i++) {
            for (int j = 0; j < World.yTiles; j++) {
                TiledMapTileLayer cur = (TiledMapTileLayer) world.map.getLayers().get("path"); //.get("map88"); //.get("Path");
                //Cell cell = new Cell();
        ...
        for (int i = 0; i < World.xTiles; i++) {
            for (int j = 0; j < World.yTiles; j++) {
                TiledMapTileLayer cur = (TiledMapTileLayer) world.map.getLayers().get("caverna");
                if (cur.getCell(i, j) != null) {
                    cur.setVisible(true);
                    int num = RandomNumber.random(min, max);
                    if (num > maiorValorEncontrado) {
    ...
    public void updatePath(boolean forceUpdate) {
        ...
        if (forceUpdate || tileX != this.lastEndTileX || tileY != this.lastEndTileY) {
            ...
            if (forceUpdate || endNode.type == FlatTiledNode.TILE_FLOOR) {
                if (endNode.type == FlatTiledNode.TILE_FLOOR) {
                    this.lastEndTileX = tileX;
                    this.lastEndTileY = tileY;
                } else {
                    endNode = worldMap.getNode(this.lastEndTileX, this.lastEndTileY);
                }
                path.clear();
